...
st.set_page_config(page_title='COVID19 Detection Tool',
...
def main():
    ...
    image_file = st.sidebar.file_uploader('Upload an X-Ray Image (jpg, png or jpeg)',
                                          type=['jpg', 'png', 'jpeg'])
    ...
    if image_file is not None:
        ...
        if choice == 'Image Enhancement':
            ...
            st.text('Original')
            st.image(new_image, width=400, use_container_width=True)
            
            new_image = cv2.cvtColor(new_image, 1) # 0: original, 1: grayscale
            st.text('???')
            st.image(new_image, width=400, use_container_width=True)
            ...
            image = image/255 # normalization: 0.0 ~ 1.0

            # For tensorflow input (n, m, p, q)
            st.text(str(image.shape))
            x_ray = image.reshape((-1, 1, 200, 200, 1))
            st.text(str(x_ray.shape))
            # x_ray = image.reshape(-1, 1, 200, 200, 1)
            ...
            # diagnosis_probability = model.predict(x_ray)
            # diagnosis = np.argmax(diagnosis_probability, axis=1)
            # #---
            ...
            # st.sidebar.info(str(diagnosis_probability) + '%')
        ...
        elif choice == 'Disclaimer and Info':
